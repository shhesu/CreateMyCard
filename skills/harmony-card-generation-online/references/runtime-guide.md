...
| 调整后生成 | 移除不可用的次要内容后，核心目标仍能满足 | 说明缺失项和保留项后自动继续 |
...
- `XX` 必须替换为用户提及的未满足内容，`YY` 必须替换为实际保留内容；“建议一”“建议二”必须替换为 Agent 根据本轮能力概述生成的 1～3 条相近需求。占位符不得原样出现在最终回复中。
...
以下话术中的 `XX` 必须替换为用户原话中对应的未满足内容，`YY` 必须替换为实际保留内容；“建议一”“建议二”由 Agent 基于本轮能力概述生成 1～3 条可直接复述的相近需求，绝不原样输出。已有合法概述时优先同领域、低风险且有完整卡片价值的建议；尚无概述时使用天气、日程、运动、设备电量或系统状态等通用示例，不承诺可用。

- 非卡片或不适合桌面卡片承载：`桌面卡片适合展示少量关键信息或提供快捷入口，暂不适合处理你这次的 XX。你可以试试：“建议一”、“建议二”`
- 核心能力无法满足：`抱歉，当前卡片能力暂无法满足你需要的 XX。你可以试试：“建议一”、“建议二”`
- 部分支持：`当前暂无法提供 XX，我会保留 YY 继续为你生成卡片。` 输出后自动继续，不等待确认；核心目标不能保留或用户明确“必须包含，否则不要生成”时终止。
...
| 数据能力部分支持 | `已为你生成卡片。本次未包含 XX 数据，保留了 YY。` |
| 部分动作缺失 | `已为你生成卡片。本次未提供 XX 操作，保留了 YY。` |
| 部分素材缺失 | `已为你生成卡片。本次未使用 XX 素材，改用了可用样式。` |
