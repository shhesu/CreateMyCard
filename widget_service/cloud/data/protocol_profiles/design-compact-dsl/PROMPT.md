...
2. 融球必须同时满足：`2x2`、单一业务、最终实际展示的独立语义信息不超过 3 项、只有一个内容组、显式动作不超过 1 个、本次运行时允许融球。有对应融球效果时优先使用；任一条件不满足都使用浅色纯色，不能先选融球再通过删减用户必须信息凑条件。列表、网格、双分区、多组独立指标、多条日程或重复事项均不是简单内容，不使用融球。
