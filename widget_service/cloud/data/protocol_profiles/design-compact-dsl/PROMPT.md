...
- 小内容蒙版统一指 2x2 S4 的 `136×64vp` 分区和 2x4 W8/W10 的 `134×59vp` 分区，只允许两行内容：第一行对象名与主数据使用 `14fp/700`、内容色 100%，第二行辅助数据固定 `12fp/400`、第十二节必要支撑文字色，不再增加独立业务标题行；需要突出量化主值时，第一行优先展示主值，但同类对象分区必须允许“对象名＋主值”；对象名不得因数字优先而省略。蒙版内 1 项数据时只输出第一行，2 项时分别放第一、二行；有语义精确匹配且状态安全的候选素材时，1-2 项使用一个右侧图标，无合法素材时不得伪造或留空槽。必要对象名与主值放不下时同样删除右侧视觉；3 项时不使用任何右侧视觉，第一行放主数据，第二行将两个辅助数据写成单个 Text 并以 ` | ` 分隔；超过 3 项时按“用户明确要求 > 主数据 > 状态 > 其他辅助”只保留前三项，其余删除。
...
2x4 每张卡必须且只能选择一个骨架，允许在声明范围内微调，不得跨骨架拼接 region。先统计数据块数量：1 个在 W1-W7 中按内容选择，2 个强制 W9，3 个强制 W10，4 个强制 W8。通用约束：独立卡级标题只允许用于单数据块骨架，有则必须使用 20vp 高的 CardHeader 作为全尺寸前景 Column 首项，无则不生成标题及空槽；W8/W9/W10 严禁卡级 title、CardHeader、公共标题行及标题空槽；分栏一律等分或固定宽（136/134/130），侧栏固定宽统一 130vp；线性进度条一律 `strokeWidth:8`；弹性沉底用包含真实内容的 `layoutWeight:1` 容器占据剩余空间（`flexShrink` 只收缩不拉伸，禁止用于沉底）；禁止 `justifyContent:"spaceBetween"` 制造三段均分（会把中部元素推离预期位置）。
...
- region：可选 `CardHeader 20vp` + 间隔2vp + `body 104`：`progressSlot 50`（主数值行 20fp/700 + label 12fp/400 + `Progress linear 276×8 strokeWidth 8`）+ 间隔8vp + `details Row 46`（两背板 134×2 gap 8、圆角 10、白色 80% 底，各「标题 12fp/400 + 值 12fp/400」）。
- 槽位：一个进度主指标 + 两个详情项。
...
- 用于：近期日程/待办 3 行清单。
- region：可选 `CardHeader 20vp` + 间隔2vp + `list 104`：三行背板 276×30（gap 7）、圆角 8、白色 80% 底；每行文本 12fp/400，不附加装饰图标。
- 槽位：恰好三行；每行一条文本。
- 禁止：两行或四行、行内按钮、行高不一。
...
- region：无卡级总标题；`root Row` 以 `width/height:matchParent` 填满 `300×150vp` 画布，padding 12、itemMargin 8，直接包含左右两个 `134×126vp` 同级背板。两背板均为 Column、padding 12、borderRadius 16、白色 80% 背板，内部可用空间均为 `110×102vp`。这是 2x4 双业务唯一合法的一级布局，不得套用 2x2 S4 的上下两个 `136×64vp` 结构，也不得放大为上下两个 `276×59vp` 背板。
- 每侧按 2x2 的层级组织；需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部按钮 110×36vp」排列；有必要对象名时先说明对象，再突出主值；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧至多保留两行最小充分信息。业务标题只是背板内分区标题，不使用 CardHeader。
...
- region：无卡级标题；`root Row` 以 `width/height:"matchParent"` 填满 `300×150vp` 画布，padding 12、itemMargin 8，直接包含左侧 `134×126vp` 大内容蒙版和右侧 `134×126vp` Column。右侧 Column 使用 itemMargin 8，上下各放一个 `134×59vp` 小内容蒙版。三个内容蒙版均使用 padding 12、borderRadius 16、白色 80% 背景。
- 左侧大内容蒙版需要突出量化主值时按「对象名 + 主读数 + 必要辅助信息 + 可选底部 Button 110×36vp」组织；没有突出数字时仍按「业务标签 12fp/400 + 主内容 + 可选底部 Button」组织。有动作时只绑定内部按钮，大内容蒙版本身不绑定，主内容最多两行最小充分信息。右侧两个分区完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。
...
- `2x4` 底部动作区只有一个卡级 Button 或图文按钮 Row 时，该按钮固定为 `x:12vp、y:102vp、width:276vp、height:36vp`，距卡片底部必须正好 `12vp`；按钮必须是满高前景 Column 的最后一项，上方真实内容区使用 `layoutWeight:1` 占据剩余空间。禁止生成 `136/130/134vp` 的半宽单按钮或浮动底边距。W9 和 W10 左侧大背板内按钮是例外，固定宽 `110vp` 并留在所属业务背板内。
