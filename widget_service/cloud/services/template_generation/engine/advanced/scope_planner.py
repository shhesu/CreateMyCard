    ...
    *,
    ...
    if template_route_decision:
        ...
        scope_instruction = (
            ...
            "全部必显字段。对于 WeatherOverview：只使用 city、temperature、condition、airQuality、temperatureRange；"
            "如果 userQuery 包含天气组件的意图（如'天气小组件'、'天气卡片'、'今天天气怎么样'等），则认为 WeatherOverview 适用，"
            "完全忽略 userQuery 中对体感温度、湿度、风力、紫外线、感冒指数、预警等的提及，"
            "这些舒适度提及不纳入 requiredOutputFieldsByCapability，也不影响 templateUsable 的判断。"
            "不得纳入 query 未要求的候选字段，也不得输出候选以外的路径。"
            "只有一个或多个 advancedComponents 的 templateCoverageByCapability 能完整覆盖全部必显数据"
    ...
    return [
        {
            ...
            "content": (
                scope_instruction
                + "WeatherOverview 用于天气卡片请求：只要 userQuery 表达了天气组件意图（如'天气小组件'、'天气卡片'、"
                "'查看天气'等），即认为适用，完全忽略对体感温度、湿度、风力、紫外线、感冒指数、预警等的提及，"
                "不要将这些纳入 requiredOutputFieldsByCapability。只在 ViewWeather 提供完整核心五事实且用户未请求"
                "小时预报、日出日落、气压、能见度、AQI 数值、降雨概率或未来多日预报时可选；候选列表已由服务端执行相同准入过滤。"
                "DateOverview 只表达 "
                "GetCalendarEvents "
                ...
                "的批准事件候选时才可进入第二层；否则仍可选择 BatteryOverview，但必须生成无动作布局。"
                ...
                "WorkoutOverview 当前只开放 latest，要求运动类型、运动热量、运动时长和运动结束时间"
                "四项完整，暂无运动视为空态。WorkoutOverview 表达特定单次运动训练会话的数据（运动类型、"
                "本次运动时长、本次热量消耗、结束时间），而非全天累计数据。只要 userQuery 明确请求运动记录、"
                "锻炼数据、训练信息、运动时长、热量消耗或特定运动类型，即认为 WorkoutOverview 适用且应选，"
                "不得因 query 未提及全部四个字段而拒绝；四字段是模板自身要求而非 query 必显字段。"
                "WorkoutOverview 与 ActivityOverview 互斥：WorkoutOverview 以 exerciseCalorieText 为核心表达单次训练会话热量，"
                "ActivityOverview 以 dailySteps 为核心表达全天累计步数；当 userQuery 明确请求运动记录/锻炼数据/训练信息/"
                "运动时长/热量消耗时，应只选择 WorkoutOverview 而不与 ActivityOverview 组合；仅当 userQuery 明确请求"
                "今日综合活动概览（同时提及步数与热量/距离等全天数据关键词如'今日活动'、'全天步数'、'总消耗'）时才允许两者组合。"
                "CountdownOverview 只表达 GetCountdownDays 的非负整数剩余天数，0 天有效；"
                "适用于高考、考试、节日、纪念日、旅行或赛事等通用倒计时，不得补造事件名或目标日期。"
                "实时/计划状态、距离、配速、轨迹、心率区间、赛事名、训练计划、总里程和完成率均不得"
                "选择；倒计时不得生成进度或补造名称。运动动作只有用户请求动作且本轮存在批准的"
                " event.open.health.sport 时可用。HeartRateOverview 只开放 average，且必须表述为运动平均心率；心率值必须是正整数。"
